...
- fix/unstructured-parsing-error（修复非结构化数据结构解析问题）
...
- **type**：提交类型，与分支命名的类型一致（feat、fix、docs、style 等）。

- **scope**：可选，指定提交影响的范围（如模块、功能、文件等），示例：feat(login)、fix(graph)。

- **subject**：提交的简短描述，不超过 50 字符，首字母小写，结尾不加句号。

- **body**：可选，详细描述提交的内容、修改原因、解决的问题等，换行时注意空行分隔。

- **footer**：可选，用于关联 Issue、关闭 Issue 或标注不兼容变更，格式：
       

    - 关联 Issue：`Related to #123`、`Refs #456`

    - 关闭 Issue：`Closes #789`、`Fixes #321`

    - 不兼容变更：`BREAKING CHANGE: 变更说明`
...
fix(graph): 修复非结构化数据结构解析问题

原因：图数据存储缺少图ID，导致数据结构解析错误。
解决方案：添加图ID字段，并完善数据结构解析逻辑。
...
- PR 描述需使用中文（除非项目明确要求英文），语言简洁、准确，避免歧义。
...
2. 克隆个人仓库到本地：`git clone https://gitee.com/qiantongtech/qKnow`。

3. 创建并切换到符合规范的分支：`git checkout -b feat/xxx`。

4. 完成代码开发和测试，提交 Commit（遵循 Commit 信息规范）。

5. 推送本地分支到个人仓库：`git push origin feat/xxx`。
...
- 本规范自发布之日起生效，后续可根据项目发展和协作需求进行修订，修订需经核心维护者讨论通过。
